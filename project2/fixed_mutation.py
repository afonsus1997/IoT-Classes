...
def mutGaussianfix(individual, mu, sigma, indpb):
...
def mutPolynomialBoundedfix(individual, eta, low, up, indpb):
...
def mutShuffleIndexesfix(individual, indpb):
    ...
    for i in range(size):
        if random.random() < indpb:
            ...
            if swap_indx >= i:
                swap_indx += 1
            individual[0][i], individual[swap_indx] = \
                individual[swap_indx], individual[0][i]

    return individual,


def mutFlipBitfix(individual, indpb):
...
def mutUniformIntfix(individual, low, up, indpb):
    ...
    for i, xl, xu in zip(range(size), low, up):
        if random.random() < indpb:
            individual[0][i] = random.randint(xl, xu)
...
def mutESLogNormalfix(individual, c, indpb):
