...
def cxPartialyMatchedfix(ind1, ind2):
...
def cxUniformPartialyMatchedfix(ind1, ind2, indpb):
...
def cxBlendfix(ind1, ind2, alpha):
...
def cxSimulatedBinaryfix(ind1, ind2, eta):
...
def cxSimulatedBinaryBoundedfix(ind1, ind2, eta, low, up):
    """Executes a simulated binary crossover that modify in-place the input
    individuals. The simulated binary crossover expects :term:`sequence`
    individuals of floating point numbers.
...
def cxMessyOnePointfix(ind1, ind2):
...
def cxESBlendfix(ind1, ind2, alpha):
...
def cxESTwoPointsfix(ind1, ind2):
    """
    .. deprecated:: 1.0
